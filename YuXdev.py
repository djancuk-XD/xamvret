    ...
    mencoba:
        ...
        dengan request.Session() sebagai (ses_dev):
            ...
            jika 'c_user' di login_dev:
                ...
                keluar()
            elif 'pos pemeriksaan' di login_dev:
                print k + '\n Akun Cekpoin...'
                keluar('\n Keluar\n')
            kalau tidak:
                print m + '\n Login Gagal...'
...
def login_dengan_cookie_():
    cetak ''
    cetak garis
    print p + ' LOGINDENGAN ' + h + ' COOKI E'
    cetak garis
    cok_in = raw_input(h + ' [' + k + 'coki' + h + ']' + p + 'Masukkan Cookie' + k +': ')
    dengan open('cookie.txt', 'w') sebagai (cookie_simpan):
        cookie_simpan.write(cok_in)
    file_cok = buka('cookie.txt', 'r').read()
    cookie = {'cookie': file_cok}
    dengan request.Session() sebagai (ses_dev):
        login = ses_dev.get(fb, cookies=cookie).content
    jika 'mbasic_logout_button' saat masuk:
        print h + '\n Login Berhasil....\n Tunggu Proses' + p + '-->>> '
        proses_masuk(cookie)
        __token__dev(kue)
        token = buka('token.txt', 'r').read()
